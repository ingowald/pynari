...
def randomColor(idx):
    ...
    idx = idx * fnv_prime ^ (idx+13)
    idx = idx * fnv_prime ^ (idx+17)
    x = (idx % 256)/255.
    idx = idx * fnv_prime ^ (idx+23)
    y = (idx % 256)/255.
    idx = idx * fnv_prime ^ (idx+29)
    z = (idx % 256)/255.
    return (x,y,z)
...
for cc in cylinder_model:
    ...
    primitive_color.append(randomColor(idx))
    vertex_color.append(randomColor(2*idx))
    vertex_color.append(randomColor(2*idx+1))
...
im = im.convert('RGB')
out_file_name = 'cylinders-colorPerPrim.png'
